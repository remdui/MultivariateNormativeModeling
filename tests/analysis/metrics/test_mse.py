...
from analysis.legacy.metrics.stats.mse import compute_mse
...
def test_mse_per_sample(data_tensors):
    ...
    mse = compute_mse(original, reconstructed, metric_type="sample")
...
def test_mse_per_feature(data_tensors):
    ...
    original, reconstructed = data_tensors
    mse = compute_mse(original, reconstructed, metric_type="feature")
...
def test_mse_total(data_tensors):
    """Test total MSE."""
    original, reconstructed = data_tensors
    mse = compute_mse(original, reconstructed, metric_type="total")
...
def test_invalid_mse_type(data_tensors):
    ...
    with pytest.raises(ValueError, match="Invalid mse_type: invalid. Choose from"):
        compute_mse(original, reconstructed, metric_type="invalid")
